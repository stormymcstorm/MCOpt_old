...
def cot(
  ...
  a_v : np.ndarray,
  b_v : np.ndarray,
  f_s : Callable[[ArrayLike], float],
  df_s : Callable[[ArrayLike], float],
  f_v : Callable[[ArrayLike], float],
  df_v : Callable[[ArrayLike], float],
  G0_s : Optional[np.ndarray] = None,
  G0_v : Optional[np.ndarray] = None,
  numItermax: int = 200,
  numItermaxEmd: int = 100000,
  verbose: bool = False,
  log: bool = False,
  ...
  if G0_s is None:
    G0_s = np.outer(a_s, b_s)
  else:
    assert G0_s.ndim == 2
    assert G0_s.shape[0] == a_s.shape[0]
    assert G0_s.shape[0] == b_s.shape[0]
  ...
  while loop:
    ...
    M_s = df_s(G_s)
    Gc_s = emd(a_s, b_s, M_s, numItermax=numItermaxEmd, log=False)
    
    deltaG_s = Gc_s - G_s
    G_s = Gc_s
    
    M_v = df_v(G_v)
    Gc_v = emd(a_v, b_v, M_v, numItermax=numItermaxEmd, log=False)
    ...
    delta = np.linalg.norm(deltaG_s) + np.linalg.norm(deltaG_v)
    
    if delta < 1e-16 or np.abs(old_fval - f_val) < 1e-7:
      break
